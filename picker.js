(function (window, document, Math) {
    ...
    Picker.prototype = {
        ...
        FillData: function () {
            ...
            //body中是否存在picker
            var modalbox = document.querySelector(".selector_mask");
            if( modalbox ){
                document.body.removeChild(modalbox);
            }

            var modalParent = document.createElement("div"); //父节点
            modalParent.className = "selector_mask";

            var picker_list = 
                '<div class="selector_select showPicker">' +
                    '<div class="ub">'+
                        '<div class="ub-f1 picker-wrapper" id="picker-wrapper0">'+
                            '<ul></ul>'+
                        '</div>';
            ...
            if( self.arrayDepth >= 2 ) {
                picker_list += 
                        '<div class="ub-f1 picker-wrapper" id="picker-wrapper1">'+
                            '<ul></ul>'+
                        '</div>';
            };
            ...
            if( self.arrayDepth >= 3 ){
                picker_list += 
                        '<div class="ub-f1 picker-wrapper" id="picker-wrapper2">'+
                            '<ul></ul>'+
                        '</div>';
            };
            ...
            picker_list += 
                    '<header>'+
                        '<button class="nav_left picker-cancel">取消</button>' +
                        '<button class="nav_right picker-ok">确定</button>'+
                    '</header>'+
                '</div>';
        ...
        showScrollList: function (dataList, index, isInit) {
            ...
            var list = '<li></li>', defaultNum, childData = [];//默认值
        ...
        HidePicker : function(){
            var self = this;
            document.querySelector('.selector_select').style.height = '0';
